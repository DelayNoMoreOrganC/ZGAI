...
- 案件进入最后办理阶段后，由律师提交结构化结案申请和依据文件，行政在审批中心核对并确认；批准后才事务性登记结案日期并开放智能归档。
- 公章用印审批支持律师上传文件、案件文书快速申请、行政待办、附件下载审阅、审批意见/驳回理由和结果留痕。
- 律所所函支持从审批通过案件预填、结构化修改与 A4 预览、DOCX 下载、用印流转，以及首次由行政定号后按年度/函种自动编号并归入案件法律文书目录。
- 同一案件可多次创建所函，草稿可取消并留痕；案件阶段允许律师填写原因后人工跳过，案件详情以账号操作日志取代办案工时记录并支持 CSV 导出。
- 案件名称/案号支持忽略常见标点、空格及案号固定文字的多词模糊检索；2026 年案件登记数据已通过可回滚工具导入 875 件，30 条差异记录留待行政补齐。
- 登录页采用广东至高律师事务所品牌主视觉、全屏动态背景和透明登录面板，并提供视频失败降级和桌面/移动端响应式布局。
...
- 案件 AI 助手无需预选案件，可在当前账号权限范围内按案号、案名、顾问单位、客户和当事人识别归属；唯一高置信案件继续处理，歧义候选由律师点选确认。明确指令可转换为日程、待办和案件进展，支持常用中文相对日期并拒绝写入过去时间；高风险阶段变化必须确认，所有动作留痕且幂等。
...
当前基线：349 项测试通过（2026-07-24 全量复验）。必须使用 JDK 11；本机默认较新 JDK 可能在 Lombok 测试编译阶段失败。
...
只回归仲裁、刑事、行政和顾问案件的两级审批、阶段跳级拦截、相邻阶段二次确认、类型化待办和财务越权时，沿用相同隔离环境变量执行：
...
2026-07-24 隔离整体验证另通过知识投稿审核桌面/手机 2 项、四类型案件流程 1 项和 RAG 桌面/手机 2 项。RAG 用例会自行创建并清理公开预期知识、禁止检索知识和评价样本，不依赖开发库预置 ID 或执行顺序；Excel 预检/导入用例仅在显式提供 `ZGAI_RAG_IMPORT_WORKBOOK` 时运行。本机 PostgreSQL 16 仍因 Homebrew bottle 元数据错误无法安装，因此真实 PostgreSQL 迁移、备份和恢复不在该基线内。

四类型阶段补充基线：仲裁、刑事、行政和顾问案件均完成隔离浏览器两级审批、待审批阶段拦截、跳级拒绝、合法相邻阶段二次确认和类型化待办；普通编辑不能伪造状态、阶段、立案、结案或归档字段，财务案件 AI 写入返回 `403`。
