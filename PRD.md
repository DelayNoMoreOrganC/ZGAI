...
5. 发票申请。
6. AI 知识库。
7. 旧系统基础资料检索与迁移底座；2026 年案件登记数据已完成首批受控导入，其他年度待用户提供整理后的数据再实施。
...
- 新建案件和案件详情均展示该类型的标准流程与律师办理重点；详情页按真实阶段状态区分待开始、进行中、已完成和已跳过。律师可手动选择任意后续阶段，跨阶段时必须填写原因，中间阶段标记为“已跳过”并写入案件日志；回退时目标之后的状态统一恢复为待开始。
...
- 立案审批通过且案件处于办理中的案件可从案件详情独立“律所所函”页面创建所函；不得为待审批、已结案或已归档案件新建所函。
- 同一案件可创建多份所函；草稿或驳回状态可由有编辑权限的律师取消，取消采用逻辑删除并写入案件日志，待审批和已批准所函不得取消。
- 系统根据案件的受理机构、委托客户、承办成员、相对方、案由、案件类型和员工联系电话生成结构化草稿；缺失内容必须由办案人补充，不得自动猜测。
- 页面提供字段编辑与 A4 同版式预览；下载 DOCX 保持用户提供模板的页边距、标题、正文、落款和页脚样式，所有原红色变量统一输出为黑色。
- 草稿可保存和下载；提交公章用印后内容锁定，审批驳回或申请人撤回后恢复编辑。用印审批继续按 `SEAL_APPROVE` 自动路由行政人员。
- 行政审批资料直接展示系统生成的 DOCX。首次使用某年度、某函种时，行政必须在同意用印动作中确定本次起始流水号；后续同年度同函种使用数据库行锁自动递增，不允许律师预占或修改正式编号。
- 审批通过日作为正式所函日期，编号格式为 `(年度)粤至高{函种}函字第{流水号}号`；编号、正式 DOCX、用印附件替换、案件文档写入和审批结果必须在同一事务边界完成。
- 正式所函写入案件 `03_法律文书`，保存版本、SHA-256、审批人和案件进展；任何生成、文件库或 NAS 写入失败均回滚审批与编号，不形成半成品正式文件。
...
每条指令保存幂等键、脱敏指令摘要、原文 SHA-256、结构化动作、风险等级、操作者、案件、执行状态和时间，新记录不得持久化指令原文。案件详情入口可锁定当前案件；全局入口无需预选案件，系统仅在当前账号有权访问的案件中，按 ZGAI 案号、法院案号、案件名称、顾问单位、委托客户和当事人识别候选。唯一高置信且可编辑的案件可以继续执行；重名、近似匹配或只读案件必须展示匹配依据并由用户确认，不得跨权限范围返回候选。动作执行器采用白名单，当前仅允许日程、待办、进展和确认后的阶段变更。阶段变更在提案前校验类型化流程，只允许进入紧邻的下一阶段；跳级或未知阶段仅返回澄清，合法提案仍不立即写入，用户必须在目标阶段及影响提示后再次确认。关联案件的日程和待办必须同时满足案件编辑权限；仅能查看案件的账号不得创建、改绑或修改案件事项。执行成功后可直接进入“日程与待办”或案件动态回看结果。三档提醒只登记尚未到达的提醒时间，避免临近开庭时一次发送多条已经过期的提醒。
...
- 2026 年案件登记表和案件清单已按用户确认的主表、编号、历史人员、共同委托人和部门映射规则完成首批受控导入；导入工具保持源文件只读，执行前后保存本地快照和校验摘要。
- 其他年度不做无差别全量导入；必须先由行政整理模板、预览差异并确认冲突，真实工作簿、数据库快照、密钥和差异清单不得提交 Git。
...
| 模块 | 已完成能力 |
|---|---|
| 登录与账号 | JWT 登录、员工账号、部门/身份类别、管理员保护、真实角色与权限分别返回（`ROLE_VIEW` 等权限不得误识别为角色）、完整员工档案与业务人员选项分离、首次登录/重置后强制改密、后端业务阻断与改密审计；响应式品牌登录页使用全屏动态背景和局部透明登录面板，视频不可用时安全降级为静态深色背景 |
...
| 案件 | 新建、列表、服务端组合筛选（名称/案号支持忽略常见标点、空格和案号固定文字的多词模糊检索，并可组合类型、状态、案由、部门、主办人、审理机构、立案日期）、详情、案件日志及 CSV 导出、回收站、列表/详情逐案有效操作权限、下次开庭日期、部门数据范围、六类案件要素/流程/办理重点、六类完整阶段与待办模板；律师可填写原因跳过阶段并留痕，待审批案件禁止推进阶段，状态/阶段/立案/结案/归档字段禁止通过普通编辑绕过专用流程；最终阶段支持结构化结案申请、依据文件、行政待办与批准后事务性结案；立案日期由主任终审实际通过日自动写入，新建和编辑不允许人工覆盖；案件详情直接显示主委托客户名称；收费方式统一显示中文业务名称，详情编辑选项与新建立案一致 |
...
| 审批 | 独立审批中心、待办、详情抽屉、消息入口、同意/驳回理由/转审、行政与主任阶段；公章用印支持真实附件、行政权限自动路由、附件审阅下载、意见留痕、申请人通知及案件文件快捷申请；律所所函支持案件预填、在线编辑预览、DOCX 生成、提交后锁定、首次流水号由行政确定、同年度同函种自动递增及批准后写入案件法律文书目录；结案复核展示案件结果、费用、客户交付、小结和可下载依据文件，批准后同步完成最后阶段和结案状态；立案审批展示当前登记的固定金额、风险金额/比例及组合收费，存量待审批单也从关联案件动态补全；审批创建先校验关联案件可见性，查询分页/排序和审批意见均有结构化校验；类型、状态、申请日期、标题/内容/申请人筛选及服务端分页已接通，已办状态先在数据库过滤再分页；越权、缺失资源和业务状态错误分别返回 403、404、400 |
...
| 案件 AI 协同 | 独立案件 AI 工作台、无需预选案件的权限内候选识别、案号/案名/顾问单位/客户/当事人匹配、歧义候选人工确认、规则引擎能力标识、指令幂等、无原文元数据摘要/SHA-256 审计、低风险自动执行、高风险确认、结构化案件进展、三档日程提醒 |
...
| 历史案件导入 | 提供只读解析源工作簿、生成预览/差异清单、停机快照、事务 SQL、引用校验和可回滚执行的导入工具；2026 年登记表以用户确认规则完成 875 件导入或合并，导入后有效案件 882 件，重复案号、孤儿当事人和孤儿成员均为 0；30 条冲突或缺号记录继续保留在本机待补清单，不进入 Git |
...
2026-07-24 最新整体验证在全新隔离 H2、临时文件根目录和私有归档 Worker 上重新执行：后端 338 项、归档 Worker 3 项、前端生产构建、四角色桌面/手机 8 项、知识投稿审核 2 项、仲裁/刑事/行政/顾问类型流程 1 项、民事与顾问完整写入闭环 2 项、RAG 桌面/手机 2 项均通过。RAG Excel 2 项因本轮未提供工作簿按设计跳过；用例已改为自行建立并清理可检索/禁止检索知识和评价样本，不再依赖固定文章 ID、空库或测试执行顺序。失败截图、trace、临时 H2 和临时案件文件均已清理。PostgreSQL 16 真实目标库迁移仍未完成：本机 `brew install postgresql@16` 再次因 Homebrew `Utils::Bottles.load_tab` 的 `undefined method '[]' for nil` 中断，未重置用户 Homebrew，也未把 H2 兼容演练误报为 PostgreSQL 验收。

macOS 试用环境新增 Finder 双击启停入口：启动入口统一复用 `start.sh` 的密钥、按需构建、旧实例优雅停止和服务启动逻辑，前后端健康检查全部通过后自动打开系统；停止入口统一复用 `stop.sh`，不得直接强制终止 Java 或覆盖运行中的 JAR。
...
| P1 | 2026 数据收尾 | 已导入或合并 875 件并完成引用校验；仍有 30 条因缺号、编号冲突或两表无法对应而暂缓 | 行政补齐差异清单后执行增量导入；不得覆盖已导入案件，不得在 Git 中保存工作簿、数据库快照或密钥 |
...
- 除已完成的 2026 年批次外，其他年度案件/客户全量导入等待用户整理数据和字段映射；2026 年剩余 30 条按差异清单单独收尾。
...
| 顺序 | 目标 | 核心工作 | 阶段出口 |
|---|---|---|---|
| 1 | 建立稳定试用底座 | 在独立 PostgreSQL 16 测试库执行迁移、回滚和恢复；在 NAS 容器验证案件文件、知识文件、LibreOffice、归档 Worker 和断线行为；真实数据快照只在受控本地/NAS 保存 | 四类真实账号可连续试用，数据库与文件备份可恢复，NAS 断线不产生半成品 |
| 2 | 收紧案件全链路 | 处理 2026 年剩余 30 条差异；完成所函创建/取消/行政定号、人工跳阶段、案件日志导出、结案及归档真实浏览器验收；为 882 件规模验证组合检索响应 | 民事与顾问案件从立案到归档各完成一条真实角色闭环，历史案件检索准确且不越权 |
| 3 | 让 AI 与知识库进入可测状态 | 模型机部署本地 Embedding，启用持久化 Qdrant；导入并审核律所制度和缺失法规附件；建立不少于 40 条正式问题及脱敏文书评价集 | RAG Top-3 命中率不低于 95%、禁止命中为 0，案件材料不进入共享 RAG，模型离线有明确降级 |
| 4 | 发布工程与安全收尾 | 收敛登录背景等外部静态依赖、前端分块与依赖漏洞；补齐 HTTPS、VPN、监控、告警、日志保留、并发和 500 页卷宗压力测试 | 形成可重复部署包、操作手册、验收报告和明确回滚方案后，再从 Demo 转正式试用 |

并行开发时，顺序 1 的数据库/权限契约为公共前置；顺序 2 和 3 可在独立 worktree 同步推进，顺序 4 由集成与 QA 分支持续跟进，不允许多个 Agent 直接修改同一工作目录。
