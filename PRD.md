...
当前自动验证基线：2026-07-24 后端 291 项测试通过，前端生产构建通过，归档 Worker 2 项生成测试通过；全部项目 Shell 脚本通过语法检查，腾讯云安全配置反例和 `git diff --check` 通过。隔离 H2 实例使用普通律师、行政、主任、财务及跨部门律师五类虚构账号完成首次改密、角色权限、客户/案件隔离、立案利冲、行政初审、主任终审、建档、文件上传、案件文书快速用印、发票申请/修订/反馈/下载/完成锁定闭环；用印与发票下载文件均通过 SHA-256 一致性校验。隔离浏览器已分别验证律师、行政、主任和财务工作台及审批/开票页面：律师默认查看本人申请且不能处理审批或上传发票反馈，行政可查看附件并同意/驳回用印，`MANAGER` 角色主任在职位写作“部门负责人”时仍有全局审批视图，财务可查看全所开票并处理反馈。新增隔离管理员浏览器验收已完成自定义角色创建、真实权限目录读取和 `CASE_VIEW + CLIENT_VIEW` 保存；员工隐私边界另以隔离管理员和律师复验，完整档案仅 `USER_VIEW` 可见，业务下拉只返回最小人员目录。隔离后端还完成标准 `SIGTERM` 与新版 `stop.sh backend` 停机实测；可执行 JAR 运行期间不得执行覆盖该文件的 `mvn package`。隔离实例不能替代真实员工账号验收。当前开发机未安装 Docker Compose CLI，也没有提交真实 `.env.nas`，因此目标 NAS Compose 解析、镜像构建和容器运行不在本次结论内。2026-07-23 使用停机生成的当前 H2 数据副本完成 PostgreSQL 兼容结构迁移演练，66 张逻辑表、11078 行数据全部通过行数和主键摘要校验；该结果不替代真实 PostgreSQL 16 目标库演练。
...
| P0 | UI/UX | 四类工作台、审批中心和发票页面已完成隔离账号桌面浏览器复验；核心导航、共享页头和案件/客户页面已按 macOS 浅色风格收敛。行政、主任、财务移动端核心流程仍缺逐页验收 | 四类工作台和案件详情、客户详情、审批、AI 核心页面逐页完成一致、可用、响应式验收 |
| P0 | E2E | 隔离 API 已覆盖建客户、立案、两级审批、建档、文件、用印和开票闭环；四类工作台及审批/开票页面已逐角色浏览器验证，但尚未形成从建客户到完成开票的可重复全浏览器流水线 | 用真实角色浏览器覆盖建客户、立案、两级审批、建档、用印和开票闭环 |
