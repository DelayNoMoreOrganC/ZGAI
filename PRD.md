...
| 模块 | 已完成能力 |
|---|---|
| 登录与账号 | JWT 登录、员工账号、部门/身份类别、管理员保护、权限集合返回、首次登录/重置后强制改密、后端业务阻断与改密审计 |
| 工作台 | 律师日历导向、行政待办导向、主任全局、财务开票导向 |
| 核心导航 | 左侧一级功能保留图标与名称，二级功能以内嵌菜单进入独立页面；律师显示日程管理，行政/财务隐藏日历；案件归档、回收站及客户/AI/知识子页保持路由高亮，移动端使用带遮罩的文字侧栏 |
...
| 系统运维 | 健康中心、审计日志、受控历史 AI 隐私脱敏、PostgreSQL 初始化脚本、H2 离线迁移工具、custom-format 备份、`pg_restore --list` 完整性校验、SHA-256、原子落盘、180 天保留策略、离线恢复演练脚本，以及腾讯云 Linux 脱敏展示/混合云两模式部署包 |
| 项目能力内化基线 | GD 首期民事归档逻辑已进入 ZGAI 原生任务流；SSB、AC 现有代码仅作为后续规则参考，不以跳转、外部 API 页面或重复账号/数据库作为交付形态 |

当前自动验证基线：2026-07-24 后端 281 项测试通过，前端生产构建通过，归档 Worker 2 项生成测试通过；全部项目 Shell 脚本通过语法检查，腾讯云安全配置测试和 NAS staging 配置校验通过，`git diff --check` 与仓库敏感密钥扫描无异常。用印专项覆盖行政权限自动路由、案件文件引用、强制审批意见、附件状态、文件哈希、跨案件拒绝和附件下载前对象可见性校验；归档专项覆盖仅结案民事案件可首次发起、已归档案件更正原因与旧成果排除、律师/行政权限分离、关键材料例外理由、重复补传及元数据失败清理、旧直接归档入口阻断、两页 PDF 页守恒/书签/清单以及图片 A4 等比转换。客户、审批、OCR、知识、AI 隐私和 PostgreSQL 兼容迁移的既有测试继续通过。当前开发机未安装 Docker Compose CLI，因此 Compose 解析、镜像构建和容器运行不在本次验证结论内。2026-07-23 使用停机生成的当前 H2 数据副本完成 PostgreSQL 兼容结构迁移演练，66 张逻辑表、11078 行数据全部通过行数和主键摘要校验；该结果不替代真实 PostgreSQL 16 目标库演练。
...
| P0 | 审批路由回归 | 权限化路由、全案件两级审批、审批筛选和服务端分页已实现并有服务/JPA测试，尚缺完成首次改密后的真实行政/主任账号浏览器回归 | 行政初审后只生成主任终审，终审前不建档、不转 ACTIVE |
| P0 | UI/UX | 核心导航、共享页头、全局搜索、系统设置深链、案件/客户列表及详情页已按 macOS 浅色风格收敛；详情页补齐加载失败、空状态、响应式描述表、移动页签和有效操作按钮。案件/客户列表完成多视口脱敏渲染；详情页当前生产构建通过，但缺真实登录账号的桌面/移动浏览器复验；其余业务页仍未形成完整设计系统 | 四类工作台和案件详情、客户详情、审批、AI 核心页面逐页完成一致、可用、响应式验收 |
