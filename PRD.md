...
当前自动验证基线：2026-07-24 后端 300 项测试通过，前端生产构建通过，归档 Worker 2 项生成测试通过，`git diff --check` 通过。隔离 H2 实例使用普通律师、行政、主任、财务及跨部门律师五类虚构账号完成首次改密、角色权限、客户/案件隔离、立案利冲、行政初审、主任终审、建档、智能归案、文件上传、案件文书快速用印、发票申请/修订/反馈/下载/完成锁定闭环；用印与发票下载文件均通过 SHA-256 一致性校验。新增受写入保护的 `scripts/e2e-approval-finance.sh`，可重复校验律师发起、越权拒绝、主任全局查看、行政用印审批、财务反馈/完成和申请人下载一致文件，未提供固定确认值时拒绝运行。隔离浏览器已分别验证律师、行政、主任和财务工作台及审批/开票页面：律师默认查看本人申请且不能处理审批或上传发票反馈，行政可查看附件并同意/驳回用印，`MANAGER` 角色主任在职位写作“部门负责人”时仍有全局审批视图，财务可查看全所开票、打开反馈上传框并处理反馈，但不再继承案件编辑/归档、客户编辑或审批处理权限。审批筛选、工作台、表格、分页及财务弹窗已补齐窄屏响应规则，行政、主任和财务的 390px 浏览器自动化验收已完成。隔离管理员浏览器验收已完成自定义角色创建、真实权限目录读取和 `CASE_VIEW + CLIENT_VIEW` 保存；员工隐私边界另以隔离管理员和律师复验，完整档案仅 `USER_VIEW` 可见，业务下拉只返回最小人员目录。隔离后端还完成标准 `SIGTERM` 与新版 `stop.sh backend` 停机实测；可执行 JAR 运行期间不得执行覆盖该文件的 `mvn package`。隔离实例不能替代真实员工账号验收。当前开发机未安装 Docker Compose CLI，也没有提交真实 `.env.nas`，因此目标 NAS Compose 解析、镜像构建和容器运行不在本次结论内。2026-07-23 使用停机生成的当前 H2 数据副本完成 PostgreSQL 兼容结构迁移演练，66 张逻辑表、11078 行数据全部通过行数和主键摘要校验；该结果不替代真实 PostgreSQL 16 目标库演练。
...
| P0 | AI 案件 E2E | 指令与归案后端、前端已完成；隔离浏览器已用生成的高分辨率中文传票图片完成真实 Tesseract OCR、模型离线降级、字段提取、候选匹配、所有权隔离、财务写入拒绝和确认归案。`/Volumes/scan` 当前未挂载，仍缺真实脱敏传票/判决书/裁定书/保全文书、真实角色、NAS/Linux 容器和扫描 PDF 回归 | 四类案件各一条，跨部门泄露为零，NAS 失败不留半成品；图片与扫描 PDF 不得返回模拟内容 |
