...
当前自动验证基线：2026-07-24 后端 289 项测试通过，前端生产构建通过，归档 Worker 2 项生成测试通过；全部项目 Shell 脚本通过语法检查，腾讯云安全配置反例和 `git diff --check` 通过。隔离 H2 实例使用普通律师、行政、主任、财务及跨部门律师五类虚构账号完成首次改密、角色权限、客户/案件隔离、立案利冲、行政初审、主任终审、建档、文件上传、案件文书快速用印、发票申请/修订/反馈/下载/完成锁定闭环；用印与发票下载文件均通过 SHA-256 一致性校验。新增隔离管理员浏览器验收已完成自定义角色创建、真实权限目录读取和 `CASE_VIEW + CLIENT_VIEW` 保存；虚构普通律师完成首次改密后读取权限目录为 `403`，直接进入行政 OA 被退回律师工作台。员工隐私边界另以隔离管理员和律师复验：管理员可读取完整员工档案，律师读取列表和详情均为 `403`，但案件主办、客户案源人等业务下拉可通过最小目录正常选择；该目录只返回人员 ID、姓名、部门和身份类别。员工管理页已实测正确显示角色、空日期和受保护开发账号。登录与当前用户响应实测只返回真实 `ADMIN/LAWYER` 角色，并将 `ROLE_VIEW/ROLE_EDIT` 正确保留为权限。律师端浏览器另已验证工作台、案件详情、客户名称、审批通过日、收费中文显示和无横向溢出。隔离后端还完成标准 `SIGTERM` 与新版 `stop.sh backend` 停机实测；可执行 JAR 运行期间不得执行覆盖该文件的 `mvn package`，正式启动脚本会先优雅停止旧实例再构建。其余行政、主任和财务已完成独立账号 API 与权限矩阵验收，但尚未完成各自浏览器页面验收；隔离实例不能替代真实员工账号验收。当前开发机未安装 Docker Compose CLI，也没有提交真实 `.env.nas`，因此目标 NAS Compose 解析、镜像构建和容器运行不在本次结论内。2026-07-23 使用停机生成的当前 H2 数据副本完成 PostgreSQL 兼容结构迁移演练，66 张逻辑表、11078 行数据全部通过行数和主键摘要校验；该结果不替代真实 PostgreSQL 16 目标库演练。
