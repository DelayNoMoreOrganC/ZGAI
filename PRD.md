...
Playwright 浏览器基线：普通律师、行政、`MANAGER` 主任和财务在 1440×900 与 390×844 共 8 项角色工作台/审批/开票检查通过；普通律师新建客户、发起立案、行政正式利冲与初审、主任终审、正式案号和审批通过日回看的 1 项写入闭环通过。全部测试使用隔离 H2 与虚构账号，不替代真实员工、PostgreSQL 或目标 NAS 验收。
...
| P0 | E2E | Playwright 已可重复运行四角色桌面/手机工作台回归，并完成首条“建客户 → 立案 → 行政利冲初审 → 主任终审 → 正式案号/立案日”浏览器写入闭环。案件文件、快速用印和开票尚未合并为同一条浏览器流水线，真实员工账号亦未验收 | 用真实角色浏览器覆盖建客户、立案、两级审批、建档、用印和开票闭环 |
