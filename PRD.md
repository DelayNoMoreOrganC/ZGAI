...
2026-07-24 最新整体验证在全新隔离 H2、临时文件根目录和私有归档 Worker 上重新执行：后端 335 项、归档 Worker 3 项、前端生产构建、四角色桌面/手机 8 项、知识投稿审核 2 项、仲裁/刑事/行政/顾问类型流程 1 项、民事与顾问完整写入闭环 2 项、RAG 桌面/手机 2 项均通过。RAG Excel 2 项因本轮未提供工作簿按设计跳过；用例已改为自行建立并清理可检索/禁止检索知识和评价样本，不再依赖固定文章 ID、空库或测试执行顺序。失败截图、trace、临时 H2 和临时案件文件均已清理。PostgreSQL 16 真实目标库迁移仍未完成：本机 `brew install postgresql@16` 再次因 Homebrew `Utils::Bottles.load_tab` 的 `undefined method '[]' for nil` 中断，未重置用户 Homebrew，也未把 H2 兼容演练误报为 PostgreSQL 验收。
