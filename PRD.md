...
| P0 | E2E | Playwright 已可重复运行四角色桌面/手机工作台回归，并完成“建客户 → 立案 → 行政利冲初审 → 主任终审 → 正式案号/立案日 → 案件文件 → 快速用印 → 行政同意 → 发票申请/修订 → 财务反馈 → 申请人下载 → 完成锁定”浏览器写入闭环。真实员工账号尚未验收 | 用真实角色浏览器复验建客户、立案、两级审批、建档、用印和开票闭环 |
