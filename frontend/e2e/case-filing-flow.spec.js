...
test('律师建客户立案并完成文件、用印、发票和电子卷宗归档闭环', async ({ browser }) => {
  ...
  const caseBeforeClosure = await authenticatedApi(lawyerPage, `/api/cases/${caseId}`)
  const stageNames = caseBeforeClosure.payload.data.stageProgress.map(stage => stage.stageName)
  const currentStageIndex = stageNames.indexOf(caseBeforeClosure.payload.data.currentStage)
  for (const targetStage of stageNames.slice(currentStageIndex + 1)) {
    const stageResponse = await authenticatedApi(lawyerPage, `/api/cases/${caseId}/status`, {
      method: 'PUT',
      body: { targetStage, reason: `E2E依次推进至${targetStage}` }
    })
    expect(stageResponse.status).toBe(200)
  }

  const caseDocuments = await authenticatedApi(lawyerPage, `/api/cases/${caseId}/documents`)
  const basisDocumentIds = caseDocuments.payload.data
    .filter(document => !document.deleted)
    .map(document => document.id)
  expect(basisDocumentIds.length).toBeGreaterThan(0)
  const closureResponse = await authenticatedApi(lawyerPage, `/api/cases/${caseId}/closure-requests`, {
    method: 'POST',
    body: {
      closureType: 'JUDGMENT',
      caseOutcome: '法院判决支持主要诉讼请求，后续履行事项已完成说明。',
      closureSummary: '本案已完成一审裁判、收费核对、客户结果告知及结案材料整理，现申请行政复核结案。',
      feeStatus: 'SETTLED',
      clientDeliveryStatus: 'COMPLETED',
      clientDeliveryNotes: '判决结果及后续安排已书面告知客户。',
      documentsConfirmed: true,
      basisDocumentIds
    }
  })
  expect(closureResponse.status).toBe(200)
  expect(closureResponse.payload.data.status).toBe('PENDING')

  const administrativeTodos = await authenticatedApi(administrativePage, `/api/todos/case/${caseId}`)
  expect(administrativeTodos.payload.data).toEqual(expect.arrayContaining([
    expect.objectContaining({ title: expect.stringContaining('[结案复核]'), status: 'PENDING' })
  ]))
  await administrativePage.goto('/approval')
  const closureCard = administrativePage.locator('.filing-card').filter({ hasText: caseName }).first()
  await expect(closureCard).toContainText('案件结案复核')
  await closureCard.getByRole('button', { name: /核对结案资料|查看审批/ }).click()
  const closureDrawer = administrativePage.locator('.approval-detail-drawer')
  await expect(closureDrawer.getByText('结案复核资料', { exact: true })).toBeVisible()
  await expect(closureDrawer.getByRole('cell', {
    name: '法院判决支持主要诉讼请求，后续履行事项已完成说明。',
    exact: true
  })).toBeVisible()
  await expect(closureDrawer.getByRole('button', { name: '下载核对' }).first()).toBeVisible()
  await closureDrawer.getByTestId('approval-approve').click()
  const closureApproveDialog = administrativePage.locator('.el-message-box')
  await closureApproveDialog.locator('input').fill('案件结果、费用、客户交付及结案依据核对无误，同意结案。')
  await closureApproveDialog.getByRole('button', { name: '确认结案', exact: true }).click()
  await expect(administrativePage.getByText('案件结案复核已通过', { exact: true })).toBeVisible()

  const closedCase = await authenticatedApi(lawyerPage, `/api/cases/${caseId}`)
  expect(closedCase.payload.data.status).toBe('CLOSED')
  const completedAdministrativeTodos = await authenticatedApi(administrativePage, `/api/todos/case/${caseId}`)
  expect(completedAdministrativeTodos.payload.data).toEqual(expect.arrayContaining([
    expect.objectContaining({ title: expect.stringContaining('[结案复核]'), status: 'COMPLETED' })
  ]))
